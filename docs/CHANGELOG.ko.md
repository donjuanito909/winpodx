...
당일 hotfix: v0.5.3 의 Debian/Ubuntu 패키지 빌드가 Ubuntu 24.04 에서 실패. 구식 `hatchling` 이 `Programming Language :: Python :: 3.14` Trove classifier 를 모르는 분류자로 거부 (known-classifier whitelist 가 3.14 stable 출시 이전). Classifier 제거; CI matrix 는 그대로 Python 3.14 기능 테스트. 코드 변경 없음.

### Highlights

**Hotfix 릴리스.** v0.5.3 의 Ubuntu 24.04 `.deb` 빌드가 `hatchling` 의 unknown-classifier 거부로 실패. `Programming Language :: Python :: 3.14` Trove classifier 한 줄 제거; CI matrix 는 그대로 3.14 테스트.

- v0.5.3 의 Debian/Ubuntu 패키지 빌드 정상화 — RPM / OBS / AUR / GitHub Release 는 영향 없음, v0.5.3 콘텐츠 그대로.
- v0.5.3 의 모든 변경 (compose timeout #212, FreeRDP launch 오류 표시 #213 / #214, Ubuntu 26.04 패키징 #206, Windows 언어 설정 #201, 동적 데스크톱 해상도 #197) 그대로 유지.

### Fixed

- **Debian/Ubuntu 패키지 빌드가 `Unknown classifier: Programming Language :: Python :: 3.14` 로 실패.** 구식 `hatchling` (Ubuntu 24.04 는 1.18.0) 이 자체 Trove classifier whitelist 를 유지하기 때문에 release 이후 추가된 classifier 를 거부. v0.5.3 의 pyproject.toml 의 3.14 선언은 cosmetic 이었고 실제 Python 3.14 커버리지는 CI matrix 에서 제공. 한 줄 제거, 기능 테스트는 그대로.

## [0.5.3] - 2026-05-19

사용자 리포트 follow-up 릴리스. 느린 회선 compose timeout (#212), #213 / #214 가 드러낸 silent "launched but no window" 실패 모드 제거, CI matrix 에 Ubuntu 26.04 + Python 3.14 추가 (#206), Windows 언어 / 지역 / 키보드 설정 (#201), 동적 데스크톱 해상도 (#197) 수정.

### Highlights

**pull 친화적 compose timeout, 가시적 FreeRDP launch 오류, CI 에 Ubuntu 26.04 + Python 3.14.** 느린 인터넷에서 첫 Windows 이미지 pull 이 더 이상 120초 벽에 걸리지 않고, silent FreeRDP 실패가 stderr 로그 경로를 출력하며, #213 / #214 를 만든 미테스트 플랫폼 공백이 닫혔습니다.

- `podman-compose up -d` / `down` 기본 timeout 120초 → 1800초; `WINPODX_COMPOSE_TIMEOUT_SECS` 환경 변수 지원 (`0` = 제한 없음). #212 수정.
- `launch_app` 가 spawn 500ms 후 프로세스를 폴링해서 stderr 로그 경로와 함께 `FreeRDP exited immediately with rc=...` raise. #213 / #214 수정.
- CI matrix 에 Ubuntu 26.04 패키지 + Python 3.14. #213 / #214 의 미테스트 플랫폼 공백을 닫음 (#206).
- Windows `[pod] language / region / keyboard` 키; `user` / `password` / `home` 와 동일한 YAML 인젝션 hardening (#201).
- 데스크톱 세션이 이제 FreeRDP 클라이언트 창 크기에 맞춰 동적으로 리사이즈 (#197).
- `.deb` 의 `/usr/share/winpodx/` 에 LICENSE / README 배포; Debian/Ubuntu 의 GUI License 탭이 다시 동작 (#201).
- GUI 시작 시 Qt 스타일시트 파서 경고 제거 (#203).
...
- CI matrix 가 이제 Python 3.14 를 테스트하고 Ubuntu 26.04 패키지도 빌드합니다 (#206). #213 / #214 에서 드러난 미테스트 플랫폼 공백을 닫습니다.
